...
# class UserState:
    ...
    # change_language = 36
    # contact_us = 40
    # write_consultant = 415


@dp.message_handler(commands=['start'])
async def on_start(message):
    user = await db_utils.get_user(message.from_user.id)
    print(f"User :{ user }")
    if not user:
        print("BEfore creating user")
        await db_utils.create_user(message.chat.id, message.from_user.id)
        print("After query")
        await send_welcome(message.chat.id)
        await get_language(message.chat.id)
    elif not user.lang:
        await get_language(message.chat.id)
    elif not user.phone_number:
        await get_phone_number(message.chat.id, user.lang)
    elif not user.is_verified:
        await send_notify(message.chat.id, user.phone_number, user.lang)
...
async def language_handler(message: types.Message):
    print("In language handler")
    user = await db_utils.get_user(message.chat.id)
    if not user:
        await send_error(message.chat.id)
...
@dp.message_handler(state=UserState.contact)
async def phone_number_handler(message):
    print("In phone_number_handler handler")
    user_id = message.from_user.id
    user = await db_utils.get_user(user_id)
    if bot.get_state(message.chat.id) != UserState.contact:
        await send_error(message.chat.id)
        return
    phone_number = await utils.get_phone_number(message.contact)
    if not phone_number:
        await send_error(message.chat.id)
        return
...
async def send_welcome(message):
    await bot.send_message(message, const.WELCOME_MESSAGE, reply_markup=utils.get_buttons(const.LIST_LANG))
